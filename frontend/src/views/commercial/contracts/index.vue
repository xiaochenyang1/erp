<template>
  <div class="page">
    ...
    <el-dialog v-model="detailVisible" :title="$t('contractPage.detailTitle')" width="1160px" @closed="closeDetail"><el-descriptions v-if="selected" :column="3" border><el-descriptions-item :label="$t('contractPage.contractNo')">{{ selected.contractNo }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.contractName')">{{ selected.contractName }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.partner')">{{ selected.customerName || selected.supplierName }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.signedDate')">{{ formatDate(selected.signedDate) }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.effectivePeriod')">{{ formatDate(selected.effectiveFrom) }} ~ {{ selected.effectiveTo ? formatDate(selected.effectiveTo) : '-' }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.totalAmount')">{{ formatMoney(selected.totalAmount) }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.remark')" :span="3">{{ selected.remark || '-' }}</el-descriptions-item></el-descriptions><el-table v-if="selected" :data="selected.lines" border stripe class="detail-table"><el-table-column prop="productCode" :label="$t('contractPage.product')" min-width="240"><template #default="{ row }">{{ `${row.productCode || ''} ${row.productName || ''}`.trim() || row.productId }}</template></el-table-column><el-table-column prop="quantity" :label="$t('contractPage.quantity')" width="110" /><el-table-column prop="committedQuantity" :label="$t('contractPage.committedQuantity')" width="125" /><el-table-column :label="$t('contractPage.availableOrderQuantity')" width="125"><template #default="{ row }">{{ availableOrderQuantity(row) }}</template></el-table-column><el-table-column prop="fulfilledQuantity" :label="$t('contractPage.fulfilledQuantity')" width="110" /><el-table-column :label="$t('contractPage.availableFulfillmentQuantity')" width="135"><template #default="{ row }">{{ availableFulfillmentQuantity(row) }}</template></el-table-column><el-table-column :label="$t('contractPage.fulfillmentProgress')" min-width="170"><template #default="{ row }"><el-progress :percentage="fulfillmentPercentage(row)" :stroke-width="10" :format="(percentage: number) => `${percentage}%`" /></template></el-table-column><el-table-column prop="unitPrice" :label="$t('contractPage.unitPrice')" width="110" /><el-table-column prop="amount" :label="$t('contractPage.amount')" width="120" /></el-table><div class="attachment-panel"><div class="attachment-header"><b>{{ $t('contractPage.attachments') }}</b><input ref="attachmentInput" type="file" hidden @change="handleAttachmentUpload" /><el-button v-permission="'contract:manage'" size="small" type="primary" @click="attachmentInput?.click()">{{ $t('contractPage.uploadAttachment') }}</el-button></div><el-table :data="attachments" size="small" border><el-table-column prop="originalFilename" :label="$t('contractPage.filename')" /><el-table-column prop="fileSize" :label="$t('contractPage.fileSize')" width="100" /><el-table-column prop="createdTime" :label="$t('contractPage.uploadedAt')" width="180" /><el-table-column width="180"><template #default="{ row }"><el-button link type="primary" @click="downloadAttachmentRow(row)">{{ $t('contractPage.downloadAttachment') }}</el-button><el-button v-permission="'contract:manage'" link type="danger" @click="deleteAttachmentRow(row)">{{ $t('contractPage.deleteAttachment') }}</el-button></template></el-table-column></el-table></div><div class="version-panel"><div class="version-header"><b>{{ $t('contractPage.versions') }}</b><span class="version-hint">{{ $t('contractPage.versionHint') }}</span></div><el-table v-loading="versionsLoading" :data="versions" size="small" border :empty-text="$t('contractPage.versionEmpty')"><el-table-column prop="versionNo" :label="$t('contractPage.versionNo')" width="80" /><el-table-column prop="eventType" :label="$t('contractPage.versionEvent')" width="130" /><el-table-column prop="status" :label="$t('contractPage.status')" width="110" /><el-table-column prop="createdTime" :label="$t('contractPage.versionTime')" width="180" /><el-table-column :label="$t('contractPage.changedFields')" min-width="200"><template #default="{ row }">{{ row.changedFields?.join(', ') || '-' }}</template></el-table-column><el-table-column :label="$t('contractPage.actions')" width="200"><template #default="{ row }"><el-button link type="primary" @click="openVersionSnapshot(row)">{{ $t('contractPage.viewVersion') }}</el-button><el-button v-permission="'contract:manage'" link type="warning" :loading="restoring" @click="restoreVersionRow(row)">{{ $t('contractPage.restoreVersion') }}</el-button></template></el-table-column></el-table></div></el-dialog>

    <el-dialog v-model="snapshotVisible" :title="snapshotTitle" width="1040px" @closed="closeSnapshot"><div v-loading="snapshotLoading" class="snapshot-body"><el-descriptions v-if="snapshot" :column="3" border><el-descriptions-item :label="$t('contractPage.contractNo')">{{ snapshot.header.contractNo }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.contractName')">{{ snapshot.header.contractName }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.partner')">{{ versionPartnerLabel(snapshot) }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.type')">{{ snapshot.header.contractType === 'SALES' ? $t('contractPage.sales') : $t('contractPage.purchase') }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.signedDate')">{{ formatDate(snapshot.header.signedDate) }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.effectivePeriod')">{{ formatDate(snapshot.header.effectiveFrom) }} ~ {{ snapshot.header.effectiveTo ? formatDate(snapshot.header.effectiveTo) : '-' }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.totalAmount')">{{ formatMoney(snapshot.header.totalAmount) }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.versionEvent')">{{ snapshot.eventType }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.status')">{{ snapshot.status }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.versionTime')">{{ snapshot.createdTime || '-' }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.changedFields')" :span="2">{{ snapshot.changedFields?.join(', ') || '-' }}</el-descriptions-item><el-descriptions-item :label="$t('contractPage.remark')" :span="3">{{ snapshot.header.remark || '-' }}</el-descriptions-item></el-descriptions><el-table v-if="snapshot" :data="snapshot.lines" size="small" border stripe class="detail-table"><el-table-column prop="lineNo" :label="$t('contractPage.versionNo')" width="70" /><el-table-column :label="$t('contractPage.product')" min-width="240"><template #default="{ row }">{{ versionProductLabel(row.productId) }}</template></el-table-column><el-table-column prop="quantity" :label="$t('contractPage.quantity')" width="110" /><el-table-column prop="fulfilledQuantity" :label="$t('contractPage.fulfilledQuantity')" width="110" /><el-table-column :label="$t('contractPage.unitPrice')" width="120" align="right"><template #default="{ row }">{{ formatMoney(row.unitPrice) }}</template></el-table-column><el-table-column :label="$t('contractPage.amount')" width="130" align="right"><template #default="{ row }">{{ formatMoney(row.amount) }}</template></el-table-column><el-table-column prop="remark" :label="$t('contractPage.remark')" min-width="140" show-overflow-tooltip /></el-table></div><template #footer><el-button @click="snapshotVisible = false">{{ $t('contractPage.cancel') }}</el-button><el-button v-if="snapshot" v-permission="'contract:manage'" type="primary" :loading="restoring" @click="restoreVersionRow(snapshot)">{{ $t('contractPage.restoreVersion') }}</el-button></template></el-dialog>
  </div>
</template>
...
const attachments = ref<Attachment[]>([]); const attachmentInput = ref<HTMLInputElement>()
...
const {
  ...
  onRestored: async () => { detailVisible.value = false; await loadData() },
  t: (key, params) => t(key, params || {})
})

const openDetail = async (row: ContractRecord) => { try { selected.value = await getContract(row.id); attachments.value = (await getContractAttachments(row.id)).records; detailVisible.value = true; await loadVersions(row.id) } catch { ElMessage.error(t('contractPage.message.detailFailed')) } }
const closeDetail = () => { selected.value = null; attachments.value = []; clearVersions() }
const openVersionSnapshot = (row: ContractVersionRecord) => { if (selected.value) void openSnapshot(selected.value.id, row) }
const restoreVersionRow = (row: ContractVersionRecord) => { if (selected.value) void restore(selected.value.id, row) }
const versionProductLabel = (productId: string) => { const product = products.value.find((item) => String(item.id) === String(productId)); return product ? `${product.productCode || ''} ${product.productName || ''}`.trim() || String(productId) : String(productId) }
const versionPartnerLabel = (record: ContractVersionRecord) => {
  if (record.header.customerId) return customers.value.find((item) => String(item.id) === record.header.customerId)?.customerName || record.header.customerId
  if (record.header.supplierId) return suppliers.value.find((item) => String(item.id) === record.header.supplierId)?.supplierName || record.header.supplierId
  return '-'
}
...
.attachment-panel { margin-top: 20px; }
.attachment-header { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 8px; }
.version-panel { margin-top: 20px; }
.version-header { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 8px; }
.version-hint { font-size: 12px; color: #909399; }
.snapshot-body { min-height: 120px; }
</style>
