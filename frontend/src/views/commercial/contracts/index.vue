    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="rows" border stripe>
        ...
        <el-table-column :label="$t('contractPage.actions')" width="390" fixed="right"><template #default="{ row }">
          ...
          <el-button v-if="row.status === 'DRAFT' || row.status === 'REJECTED'" v-permission="'contract:manage'" link type="success" @click="runAction(submitContract, row)">{{ $t('contractPage.submit') }}</el-button>
          <el-button v-if="row.status === 'SUBMITTED'" v-permission="'contract:approve'" link type="success" @click="runAction(approveContract, row)">{{ $t('contractPage.approve') }}</el-button>
          <el-button v-if="row.status === 'SUBMITTED'" v-permission="'contract:approve'" link type="warning" @click="runAction(rejectContract, row)">{{ $t('contractPage.reject') }}</el-button>
          <el-button v-if="row.status === 'ACTIVE'" v-permission="'contract:approve'" link type="warning" @click="runAction(closeContract, row)">{{ $t('contractPage.close') }}</el-button>
          <el-button v-if="['DRAFT', 'SUBMITTED', 'REJECTED', 'ACTIVE'].includes(row.status)" v-permission="'contract:manage'" link type="danger" @click="runAction(cancelContract, row)">{{ $t('contractPage.cancelContract') }}</el-button>
...
const runAction = async (action: (id: string) => Promise<unknown>, row: ContractRecord) => { try { await ElMessageBox.confirm(t('contractPage.message.confirmAction', { action: t('contractPage.message.actionDone') }), t('contractPage.message.prompt'), { type: 'warning' }); await action(row.id); ElMessage.success(t('contractPage.message.actionDone')); await loadData() } catch (error: any) { if (error !== 'cancel' && error !== 'close') ElMessage.error(t('contractPage.message.actionFailed')) } }
const handleExport = async () => { try { const blob = await exportContracts(query); downloadBlob(blob, 'contracts.csv'); ElMessage.success(t('contractPage.message.exported')) } catch { ElMessage.error(t('contractPage.message.actionFailed')) } }
