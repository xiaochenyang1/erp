...
**更新时间**：2026-09-23（多币种汇兑损益、报表口径、期末调汇与期初导入币种提交后）
...
- **功能验证通过**；
- **本轮多币种收尾已按功能提交到本分支，但这些提交上还没有重跑全量回归和浏览器 UI smoke**；
...
### 2026-09-18 汇兑损益与报表口径状态

多币种阶段一的第 5 步「汇兑损益和报表口径」此前只有一份未提交的 V164 科目种子，没有任何代码引用；V161 给应收应付和核销明细加的本位币列也从未被写入。本轮把这一步补完：

- 应收应付建账写入来源单据的币种/汇率快照与本位币原始金额；
- 采购入库、采购退货、销售出库、销售退货一律按本位币过账（此前外币单据按原币记账，1122/2202 永远轧不平）；
- 收付款核销落库 `base_amount`/`base_settled_amount`/`fx_gain_loss_amount`，并按「结算汇率 − 入账汇率」生成 6061 汇兑损益凭证腿，作废按落库快照精确冲回；
- 核销强制币种一致；
- 账龄与结算报表切到本位币口径并给出币种敞口，新增 `/api/reports/fx-gain-loss` 汇兑损益报表与报表中心标签页。

**仍未完成的发布动作**：期末未实现汇兑损益重估已实现（V166、`POST /api/finance/periods/{id}/fx-revaluation`、下期初红冲、撤销、月结检查）。2026-09-23 工作区默认测试组在一次性 MySQL 8.4 上为 2277 项、3 skipped；唯一失败是新表 `fin_fx_revaluation` 未进入租户拦截清单，已补进 `TENANT_TABLES` 并复跑覆盖测试通过。同一工作区另起一次性库启动当前代码后，扩展 API smoke 16/16，期间调汇完成首次过账、重复调用和撤销后再调汇。随后又把业务追踪的单据金额、时间线和未结应收应付合计，以及逾期异常扫描的剩余金额，改成与看板相同的本位币口径；期初应收应付导入模板增加可选 `currency_code`/`exchange_rate`，旧表头仍可导入并按本位币记账。这些改动已提交到 `refactor/postrelease-sql-gate`，但还没有在该 HEAD 上重跑全量回归和浏览器 UI smoke，也没有替代财务、质检、运维和真实预生产门禁。
