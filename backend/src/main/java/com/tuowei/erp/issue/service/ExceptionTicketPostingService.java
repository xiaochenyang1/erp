...
/**
 * 异常工单写侧：创建、分派、状态流转（开始/解决/关闭）、超时升级。
 *
 * 状态机：OPEN →start→ PROCESSING →resolve→ RESOLVED →close→ CLOSED；已关闭不可改。
 * 工单号按租户+业务日单日递增（ET-yyyyMMdd-NNNN）。超时升级受 SLA 策略开关与单次升级约束。
 * 读侧通过 {@link ExceptionTicketQueryService} 完成租户边界校验与响应映射。
 */
@Service
public class ExceptionTicketPostingService {
    ...
    public ExceptionTicketPostingService(
    ...
    public ExceptionTicketResponse create(ExceptionTicketCreateRequest request, AuditMetadata audit) {
        ...
        return exceptionTicketQueryService.toResponse(ticket, List.of(event));
    }

    @Transactional
    public ExceptionTicketResponse assign(Long id, ExceptionTicketAssignRequest request) {
        AuditMetadata audit = auditMetadataFactory.current();
        ExceptionTicketEntity ticket = exceptionTicketQueryService.requireTicket(id, audit);
        rejectClosed(ticket);
        LocalDateTime now = audit.now();
