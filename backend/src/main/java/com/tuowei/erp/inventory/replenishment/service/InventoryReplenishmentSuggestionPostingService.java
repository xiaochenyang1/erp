...
/**
 * 补货建议写侧：创建（低库存规则校验 + 现存量校验 + 草稿去重 + 插入 + 预警处置）、
 * 更新、取消、转采购订单（采购订单创建 + 状态翻转 + 乐观锁）。
 *
 * 通过 {@link InventoryReplenishmentSuggestionQueryService#requireSuggestion} 复用租户边界加载，
 * 通过 {@link InventoryReplenishmentSuggestionQueryService#toResponse} 复用响应映射，状态机与
 * 采购订单转单保持单一外层事务。
 */
@Service
public class InventoryReplenishmentSuggestionPostingService {
    ...
    public InventoryReplenishmentSuggestionPostingService(
    ...
    public InventoryReplenishmentSuggestionResponse create(InventoryReplenishmentSuggestionCreateRequest request) {
        ...
        return replenishmentSuggestionQueryService.toResponse(entity, warehouse, product, supplier, null);
    }
    ...
    public InventoryReplenishmentSuggestionResponse update(
        ...
        InventoryReplenishmentSuggestionEntity entity = replenishmentSuggestionQueryService.requireSuggestion(id);
        ...
        return replenishmentSuggestionQueryService.toResponse(entity);
    }
