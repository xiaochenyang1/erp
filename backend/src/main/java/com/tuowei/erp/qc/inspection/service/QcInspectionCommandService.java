...
/** Write-side commands for quality inspection edits, submissions and judgments. */
@Service
public class QcInspectionCommandService {
    ...
    public QcInspectionCommandService(
    ...
    /** OQC 判定：gate-only，只落合格/不合格结果，不回写销售出库行。 */
    ...
    private QcInspectionOrderEntity requireInspection(Long id, AuditMetadata audit) {
        return qcInspectionQueryService.requireInspection(id, audit);
    }

    private List<QcInspectionLineEntity> loadInspectionLines(QcInspectionOrderEntity inspection) {
        return qcInspectionQueryService.loadInspectionLines(inspection);
    }

    private QcInspectionResponse getById(Long id) {
