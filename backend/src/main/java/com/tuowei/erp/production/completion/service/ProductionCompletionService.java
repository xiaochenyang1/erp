...
public class ProductionCompletionService {
    ...
    @Transactional
    public ProductionOrderResponse complete(Long orderId) {
        return complete(orderId, null);
    }
    ...
    public ProductionOrderResponse complete(Long orderId, ProductionCompletionRequest request) {
        ...
        BigDecimal completionQty = request != null && request.completedQty() != null
                ? ScalePrecision.quantity(request.completedQty())
                : remainingPlannedQty;
        if (completionQty.compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("生产完工数量必须大于0");
        }
        if (completionQty.compareTo(remainingPlannedQty) > 0) {
            throw new IllegalArgumentException("生产完工数量超过剩余计划数量");
        }
        BigDecimal maxCompletableQty = maxCompletableQty(order);
        BigDecimal remainingCompletableQty = ScalePrecision.quantity(maxCompletableQty.subtract(ScalePrecision.zeroDefault(order.getCompletedQty())));
        if (completionQty.compareTo(remainingCompletableQty) > 0) {
            throw new IllegalArgumentException("生产完工数量超过已领料可完工数量");
        }
        productionOperationService.assertReadyForCompletion(order, completionQty);
        qcInspectionGate.assertProductionInspected(
                order.getCompanyId(),
    ...
    private BigDecimal maxCompletableQty(ProductionOrderEntity order) {
        ...
        for (ProductionOrderMaterialEntity material : materials) {
            ...
            BigDecimal materialCompletableQty = ScalePrecision.quantity(
                    ScalePrecision.zeroDefault(material.getIssuedQty())
                            .multiply(order.getPlannedQty())
                            .divide(requiredQty, 8, RoundingMode.HALF_UP)
            );
            if (materialCompletableQty.compareTo(maxQty) < 0) {
                maxQty = materialCompletableQty;
            }
        }
        return maxQty;
    }
    ...
    private BigDecimal completionAmount(BigDecimal availableAmount, BigDecimal remainingCompletableQty, BigDecimal completionQty) {
        if (remainingCompletableQty.compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("没有可结转的生产成本，不能完工");
        }
        if (completionQty.compareTo(remainingCompletableQty) == 0) {
            return ScalePrecision.amount(availableAmount);
        }
