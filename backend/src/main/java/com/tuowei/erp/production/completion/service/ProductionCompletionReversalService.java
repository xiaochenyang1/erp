...
public class ProductionCompletionReversalService {
    ...
    public ProductionCompletionReversalService(
            ...
            ProductionCompletionReversalMapper reversalMapper,
    ...
    public ProductionOrderResponse reverseCompletion(Long orderId, ProductionCompletionReversalRequest request) {
        ...
        if (completedQty.compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("生产工单没有可冲回的完工数量");
        }
        ...
        BigDecimal reversedQty = request != null && request.reversedQty() != null
                ? ScalePrecision.quantity(request.reversedQty())
                : completedQty;
        if (reversedQty.compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("生产完工冲回数量必须大于0");
        }
        if (reversedQty.compareTo(completedQty) > 0) {
            throw new IllegalArgumentException("生产完工冲回数量超过已完工数量");
        }
        ...
        BigDecimal reversedAmount = inventoryPostingService.postOutbound(
                new InventoryPostingCommand(
                        order.getFinishedWarehouseId(),
                        order.getProductId(),
                        BIZ_TYPE,
                        reversal.getReversalNo(),
                        reversal.getId(),
                        reversedQty,
                        BigDecimal.ZERO,
                        actionRemark,
        ...
        if (reversalMapper.updateById(reversal) != 1) {
            throw new BusinessConflictException("生产反完工单已被其他操作修改，请重试");
        }

        BigDecimal newCompletedQty = ScalePrecision.quantity(completedQty.subtract(reversedQty));
        BigDecimal newFinishedAmount = ScalePrecision.amount(
                ScalePrecision.zeroDefault(order.getFinishedAmount()).subtract(reversedAmount)
        );
        if (newFinishedAmount.compareTo(BigDecimal.ZERO) < 0) {
            newFinishedAmount = ScalePrecision.amount(BigDecimal.ZERO);
        }
        ...
        financePostingService.recordProductionCompletionReversal(
                order,
                reversal.getId(),
                reversal.getReversalNo(),
                reversal.getReversedAmount(),
                reversalDate,
                audit
        );
