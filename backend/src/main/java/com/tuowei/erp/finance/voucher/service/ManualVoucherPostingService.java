...
/**
 * 手工凭证写侧：草稿 CRUD、状态流转、过账与红冲作废。
 *
 * 状态机：DRAFT →submit→ PENDING →approve→ APPROVED →post→ POSTED →cancel→ CANCELLED；
 *          PENDING →reject→ DRAFT。仅 DRAFT 可编辑/删除；过账、作废受期间锁定约束。
 *
 * 过账时把分录灌入共享 fin_voucher + fin_voucher_entry（与自动凭证同源），作废时另生成红冲凭证
 * 使总账抵销，原始凭证和分录均保留。读侧通过 {@link ManualVoucherQueryService} 完成租户边界校验。
 */
@Service
public class ManualVoucherPostingService {
    ...
    public ManualVoucherResponse update(Long id, ManualVoucherSaveRequest request) {
        AuditMetadata audit = auditMetadataFactory.current();
        LocalDateTime now = audit.now();
        ManualVoucherEntity voucher = manualVoucherQueryService.requireVoucher(id, audit);
    ...
    public void submit(Long id) {
        ...
        requireBalanced(manualVoucherQueryService.loadLines(voucher, audit));
    ...
    public void post(Long id) {
        ...
        List<ManualVoucherLineEntity> lines = manualVoucherQueryService.loadLines(voucher, audit);
