...
/** Read-side query operations for expense management. */
@Service
public class ExpenseQueryService {
    ...
    public PageResponse<ExpenseResponse> list(ExpensePageQuery query) {
        ...
        LambdaQueryWrapper<ExpenseEntity> wrapper = buildListQuery(audit, safeQuery);
    ...
    public ExpenseReconciliationResponse reconciliation(Long id) {
        ...
        boolean amountMatched = voucher != null
    ...
    private LambdaQueryWrapper<ExpenseEntity> buildListQuery(AuditMetadata audit, ExpensePageQuery query) {
        ...
        if (StringUtils.hasText(query.getStatus())) {
            wrapper.eq(ExpenseEntity::getStatus, query.getStatus().trim().toUpperCase(Locale.ROOT));
        }
        if (query.getDateFrom() != null) {
            wrapper.ge(ExpenseEntity::getExpenseDate, query.getDateFrom());
        }
        if (query.getDateTo() != null) {
            wrapper.le(ExpenseEntity::getExpenseDate, query.getDateTo());
        }

        return wrapper.orderByDesc(ExpenseEntity::getExpenseDate).orderByDesc(ExpenseEntity::getId);
    }
