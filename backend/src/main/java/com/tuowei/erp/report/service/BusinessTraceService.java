...
public class BusinessTraceService {
    ...
    private final BusinessTraceDataScopeService traceDataScopeService;
    ...
    public BusinessTraceService(
            ...
            BusinessTraceDataScopeService traceDataScopeService,
        ...
        this.traceDataScopeService = traceDataScopeService;
    ...
    public BusinessTraceService(
        ...
        this(currentUserContext, new BusinessTraceDataScopeService(dataScopeService), scopedUserResolver,
                purchaseOrderMapper, salesOrderMapper, purchaseReceiptMapper, salesDeliveryMapper, payableMapper,
                receivableMapper, inventoryTransactionMapper, workflowTaskMapper, operationLogMapper,
                exceptionTicketMapper, clock, assemblyService);
    }
    ...
    private List<SalesOrderEntity> listSalesOrders(String keyword, ScopedUsers scopedUsers) {
        return salesOrderMapper.selectList(traceDataScopeService.salesOrderScope(
                        scopedUsers.currentUser(), scopedUsers.snapshot(), scopedUsers.scopedUserIds())
    ...
    private List<PurchaseOrderEntity> listPurchaseOrders(String keyword, ScopedUsers scopedUsers) {
        return purchaseOrderMapper.selectList(traceDataScopeService.purchaseOrderScope(
                        scopedUsers.currentUser(), scopedUsers.snapshot(), scopedUsers.scopedUserIds())
    ...
    private List<SalesDeliveryEntity> listSalesDeliveries(String keyword, Set<Long> orderIds, ScopedUsers scopedUsers) {
        LambdaQueryWrapper<SalesDeliveryEntity> wrapper = traceDataScopeService.salesDeliveryScope(
                        scopedUsers.currentUser(), scopedUsers.snapshot(), scopedUsers.scopedUserIds())
    ...
    private List<PurchaseReceiptEntity> listPurchaseReceipts(String keyword, Set<Long> orderIds, ScopedUsers scopedUsers) {
        LambdaQueryWrapper<PurchaseReceiptEntity> wrapper = traceDataScopeService.purchaseReceiptScope(
                        scopedUsers.currentUser(), scopedUsers.snapshot(), scopedUsers.scopedUserIds())
    ...
    private List<ReceivableEntity> listReceivables(String keyword, Set<String> knownBizNos, ScopedUsers scopedUsers) {
        var scopedQuery = traceDataScopeService.receivableQuery(
                scopedUsers.currentUser(), scopedUsers.snapshot(), keyword, knownBizNos);
        if (scopedQuery.isEmpty()) {
            return List.of();
        }
        return receivableMapper.selectList(scopedQuery.orElseThrow()
    ...
    private List<PayableEntity> listPayables(String keyword, Set<String> knownBizNos, ScopedUsers scopedUsers) {
        var scopedQuery = traceDataScopeService.payableQuery(
                scopedUsers.currentUser(), scopedUsers.snapshot(), keyword, knownBizNos);
        if (scopedQuery.isEmpty()) {
            return List.of();
        }
        return payableMapper.selectList(scopedQuery.orElseThrow()
    ...
    private List<InventoryTransactionEntity> listInventoryTransactions(String keyword, Set<String> knownBizNos, ScopedUsers scopedUsers) {
        LambdaQueryWrapper<InventoryTransactionEntity> wrapper = traceDataScopeService.inventoryTransactionScope(
                scopedUsers.currentUser(), scopedUsers.snapshot());
        wrapper.and(nested -> {
    ...
    private List<WorkflowTaskEntity> listWorkflowTasks(String keyword, Set<String> knownBizNos, ScopedUsers scopedUsers) {
        var scopedQuery = traceDataScopeService.workflowTaskQuery(
                scopedUsers.currentUser(), scopedUsers.snapshot(), keyword, knownBizNos);
        if (scopedQuery.isEmpty()) {
            return List.of();
        }
        return workflowTaskMapper.selectList(scopedQuery.orElseThrow()
    ...
    private List<OperationLogEntity> listOperationLogs(String keyword, Set<String> knownBizNos, ScopedUsers scopedUsers) {
        var scopedQuery = traceDataScopeService.operationLogQuery(
                scopedUsers.currentUser(), scopedUsers.snapshot(), keyword, knownBizNos);
        if (scopedQuery.isEmpty()) {
            return List.of();
        }
        return operationLogMapper.selectList(scopedQuery.orElseThrow()
    ...
    private List<ExceptionTicketEntity> listExceptionTickets(String keyword, Set<String> knownBizNos, ScopedUsers scopedUsers) {
        var scopedQuery = traceDataScopeService.exceptionTicketQuery(
                scopedUsers.currentUser(), scopedUsers.snapshot(), keyword, knownBizNos);
        if (scopedQuery.isEmpty()) {
            return List.of();
        }
        return exceptionTicketMapper.selectList(scopedQuery.orElseThrow()
    ...
    private ScopedUsers scopedUsers(CurrentUser currentUser) {
        ...
        return new ScopedUsers(currentUser, snapshot, ids);
    }
