package com.tuowei.erp.sales.order.web;

import jakarta.validation.constraints.DecimalMin;
import jakarta.validation.constraints.NotNull;

import java.math.BigDecimal;

public record SalesOrderLineRequest(
        Long contractLineId,
        @NotNull(message = "productId不能为空") Long productId,
        @NotNull(message = "qty不能为空")
        @DecimalMin(value = "0.0001", message = "qty必须大于0") BigDecimal qty,
        BigDecimal auxQty,
        String auxUnitName,
        BigDecimal conversionFactor,
        @NotNull(message = "price不能为空")
        @DecimalMin(value = "0.00", message = "price不能小于0") BigDecimal price,
        @NotNull(message = "taxRate不能为空")
        @DecimalMin(value = "0.00", message = "taxRate不能小于0") BigDecimal taxRate,
        String remark
) {
    public SalesOrderLineRequest(Long productId, BigDecimal qty, BigDecimal price, BigDecimal taxRate, String remark) {
        this(null, productId, qty, null, null, null, price, taxRate, remark);
    }

    public SalesOrderLineRequest(Long productId, BigDecimal qty, BigDecimal auxQty, String auxUnitName,
                                 BigDecimal conversionFactor, BigDecimal price, BigDecimal taxRate, String remark) {
        this(null, productId, qty, auxQty, auxUnitName, conversionFactor, price, taxRate, remark);
    }
}
