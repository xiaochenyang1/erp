...
/** Read-side data scope, detail loading and response mapping for sales orders. */
@Service
public class SalesOrderQueryService {
    ...
    public PageResponse<SalesOrderResponse> list(SalesOrderPageQuery query) {
        ...
        Page<SalesOrderEntity> result = salesOrderMapper.selectPage(page, scopedListQuery(safeQuery));
    ...
    public SalesOrderResponse getById(Long id) {
        ...
        List<SalesOrderLineEntity> lines = salesOrderLineMapper.selectList(
    ...
    private LambdaQueryWrapper<SalesOrderEntity> scopedListQuery(SalesOrderPageQuery query) {
        CurrentUser currentUser = currentUserContext.requireCurrentUser();
        DataScopeSnapshot snapshot = currentUserContext.requirePrincipal().dataScopeSnapshot();
        ScopedUserResolver.ScopedUserIds scopedUserIds = scopedUserResolver.resolve(currentUser, snapshot);
        LambdaQueryWrapper<SalesOrderEntity> wrapper = buildListQuery(
