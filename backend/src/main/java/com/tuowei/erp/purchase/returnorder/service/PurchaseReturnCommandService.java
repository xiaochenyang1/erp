...
/** Creation, editing and cancellation commands for purchase returns. */
@Service
public class PurchaseReturnCommandService {
    ...
    public PurchaseReturnCommandService(
    ...
    public PurchaseReturnResponse update(Long id, PurchaseReturnUpdateRequest request) {
        ...
        if (!Objects.equals(entity.getReceiptId(), request.receiptId())) {
            throw new IllegalArgumentException("采购退货单不允许变更来源采购入库单");
        }
        PurchaseReceiptEntity receipt = requirePostedReceipt(entity.getReceiptId(), entity.getCompanyId(), entity.getAccountBookId());
        assertCanView(receipt);
        Map<Long, PurchaseReceiptLineEntity> receiptLines = loadReceiptLines(receipt);
        AuditMetadata audit = auditMetadataFactory.current();
        LocalDateTime now = audit.now();
    ...
    private List<PurchaseReturnLineEntity> saveLines(Long returnId, List<PurchaseReturnLineRequest> requests,
        ...
        List<PurchaseReturnLineEntity> lines = new ArrayList<>();
        AccumulatedQuantityValidator quantityValidator = new AccumulatedQuantityValidator("退货数量超过可退数量");
        List<Long> productIds = requests.stream().map(r -> requireReceiptLine(receiptLines, r.receiptLineId()).getProductId()).toList();
        ...
        for (int i = 0; i < requests.size(); i++) {
            ...
            line.setReturnId(returnId); line.setLineNo(i + 1); line.setReceiptLineId(receiptLine.getId());
            line.setOrderLineId(receiptLine.getOrderLineId()); line.setProductId(receiptLine.getProductId());
            PurchaseAmountCalculator.LineAmounts amounts = PurchaseAmountCalculator.line(qty, receiptLine.getPrice(), receiptLine.getTaxRate());
            ...
            PurchaseReturnLineViewData.from(receiptLine, productMap.get(receiptLine.getProductId())).applyTo(line);
            ...
            line.setSerialNos(request.serialNos()); line.setRemark(request.remark());
            ...
            purchaseReturnLineMapper.insert(line); lines.add(line);
        }
        return lines;
    }
    ...
    private PurchaseReceiptEntity requirePostedReceipt(Long id, Long companyId, Long accountBookId) {
        ...
        if (!Objects.equals(entity.getCompanyId(), companyId) || !Objects.equals(entity.getAccountBookId(), accountBookId)) {
            throw new IllegalArgumentException("采购入库单未过账，不能创建采购退货单");
        }
        return entity;
    }
    ...
    private Map<Long, PurchaseReceiptLineEntity> loadReceiptLines(PurchaseReceiptEntity receipt) {
        return purchaseReceiptLineMapper.selectList(new LambdaQueryWrapper<PurchaseReceiptLineEntity>()
    ...
    private PurchaseReceiptLineEntity requireReceiptLine(Map<Long, PurchaseReceiptLineEntity> receiptLines, Long receiptLineId) {
        ...
        if (entity == null) throw new IllegalArgumentException("采购入库单明细不存在");
        return entity;
    }
    ...
    private void assertCanView(PurchaseReturnEntity entity) { purchaseReturnQueryService.assertCanView(entity); }
    private void assertCanView(PurchaseReceiptEntity entity) { purchaseReturnQueryService.assertCanView(entity); }
}
