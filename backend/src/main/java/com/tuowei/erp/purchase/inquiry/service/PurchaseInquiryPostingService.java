...
/**
 * Write-side commands for purchase inquiries: create, update, submit, quote entry, winner selection,
 * conversion to purchase order, and cancellation. Read-side filtering and response mapping live in
 * {@link PurchaseInquiryQueryService}; quote persistence lives in {@link PurchaseInquiryQuoteService}.
 * Transactions are held here so read-after-write tail calls route through the query service within the
 * same transaction.
 */
@Service
public class PurchaseInquiryPostingService {
    ...
    public PurchaseInquiryPostingService(
    ...
    public PurchaseInquiryResponse update(Long id, PurchaseInquiryUpdateRequest request) {
        ...
        List<PurchaseInquiryLineEntity> lines = insertLines(inquiry, request.lines(), audit, now);
        return purchaseInquiryQueryService.toResponse(inquiry, lines, purchaseInquiryQuoteService.loadQuoteResponses(inquiry));
    }
    ...
    public PurchaseOrderResponse convertToPurchaseOrder(Long id) {
        AuditMetadata audit = auditMetadataFactory.current();
        PurchaseInquiryEntity inquiry = purchaseInquiryQueryService.requireInquiryForUpdate(id, audit);
